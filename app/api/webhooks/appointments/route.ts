...
// Node runtime required — React Email's render() uses Node APIs.
...
export async function POST(req: NextRequest) {
  ...
  // Hydrate patient + provider in parallel
  ...
  const ctx: EmailContext = { kind, appointment, patient, provider, recipients };
  const result = await sendNotification(ctx);

  if (!result.ok) {
    // Return 200 so Supabase doesn't retry a known-bad payload indefinitely.
    // The notification_events row records the failure for follow-up.
    return NextResponse.json({ ok: false, error: result.error });
  }

  return NextResponse.json({ ok: true, kind, recipients, messageId: result.messageId });
}
