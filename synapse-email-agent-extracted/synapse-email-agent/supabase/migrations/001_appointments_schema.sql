...
create table if not exists providers (
  ...
  last_name text not null,
  specialty text,
  email text not null,            -- where notifications can also go
...
create table if not exists appointments (
  ...
  created_by_agent text,          -- e.g. 'scheduling_agent_v1'
  agent_reasoning text,           -- frozen at-decision-time summary (PRD §6 Explainability Drawer)
  agent_confidence numeric(4,3),  -- 0.000..1.000

  -- Lifecycle audit fields
  reschedule_reason text,         -- populated when an update changes starts_at/ends_at
  cancellation_reason text,       -- populated when status → cancelled
  previous_starts_at timestamptz, -- snapshot of last starts_at (set by trigger on reschedule)
...
-- ============================================================================
-- Trigger: capture previous times on reschedule
-- ============================================================================
...
-- Row-Level Security
-- ============================================================================
-- For pilot: enable RLS but allow service-role bypass (the webhook runs as service role).
-- Tighten per-role policies in a later migration once auth is wired.
...
-- Service role has full access (used by webhook + admin tools).
-- Add user-facing policies in a follow-up migration after Supabase Auth is set up.
