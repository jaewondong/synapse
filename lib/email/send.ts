 ...
 * Logs are written even on failure so the audit trail is always complete.
 */
export async function sendNotification(ctx: EmailContext): Promise<{
  ...
  // Insert a 'queued' row first — paper trail even if the send blows up.
  ...
  try {
    ...
    if (result.error) {
      await updateLog(logRow?.id, { status: "failed", error: result.error.message });
...
async function updateLog(id: string | undefined, patch: Record<string, unknown>) {
  if (!id) return;
...
/** Decide whether an UPDATE event warrants a notification and which kind. */
